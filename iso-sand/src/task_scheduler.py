...
class TaskScheduler:
    ...
    def __init__(self, event_file: str = _DEFAULT_EVENT_FILE,
        ...
        # Phase 2: 支持启动时直接从 tasks.yaml 加载（不再硬编码空列表）
        if tasks_file:
            self._tasks = _load_tasks_from_yaml(tasks_file)
            print(f"[task_scheduler] 📋 已从 {tasks_file} 加载 {len(self._tasks)} 个任务")
...
def _load_tasks_from_yaml(yaml_path: str = None) -> list:
    """
    从 tasks.yaml 加载任务配置（Phase 2：调度器接真实任务，替换硬编码空列表）。

    参数:
        yaml_path: tasks.yaml 路径（默认: iso-sand/deploy/tasks.yaml）

    返回:
        任务列表 [{"id": ..., "cron": ..., "command": ..., "timeout": ...}, ...]

    规则:
        - 占位符替换: {CODE_DIR}=iso-sand根, {SRC_DIR}=iso-sand/src,
          {DEPLOY_DIR}=iso-sand/deploy, {DATA_DIR}=iso-sand/data, {BASE_DIR}=上层目录
        - tasks.yaml 的 schedule 键 → 内部 cron 键
        - 目标校验: command 指向的脚本文件不存在 → WARN 并跳过该任务（不接必失败任务）
    """
    ...
    if not os.path.exists(yaml_path):
        print(f"[task_scheduler] ⚠️ tasks.yaml not found: {yaml_path}")
        return []
    ...
    tick_interval = (data.get("scheduler") or {}).get("tick_interval")
    if tick_interval:
        print(f"[task_scheduler] ⏱️ tasks.yaml 指定 tick_interval={tick_interval}s")

    # 参数化路径（iso-sand 布局）
    iso_sand_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    src_dir = os.path.join(iso_sand_root, "src")
    deploy_dir = os.path.join(iso_sand_root, "deploy")
    base_dir = os.path.dirname(iso_sand_root)
    data_dir = os.path.join(iso_sand_root, "data")

    loaded = []
    for task in tasks:
        cmd = task.get("command", "")
        cmd = cmd.replace("{CODE_DIR}", iso_sand_root)
        cmd = cmd.replace("{SRC_DIR}", src_dir)
        cmd = cmd.replace("{DEPLOY_DIR}", deploy_dir)
        ...
        # 目标文件存在性校验：python3 <script> 形式的命令，脚本缺失 → 跳过不接
        # （路径可能带引号——含空格路径如 "Agent OS/..."，校验时剥离引号）
        parts = cmd.strip().split()
        if len(parts) >= 2 and parts[0] in ("python", "python3"):
            script = parts[1].strip("'\"")
            if script.endswith(".py") and not os.path.exists(script):
                print(f"[task_scheduler] ⚠️ 任务 '{task.get('id')}' 目标脚本不存在 → 跳过不接: "
                      f"{script}")
                continue
        loaded.append(task)

    print(f"[task_scheduler] ✅ 加载 {len(loaded)} 个定时任务（跳过 {len(tasks) - len(loaded)} 个）")
    return loaded
...
def main():
    ...
    parser.add_argument("--tasks-file", default=None, help="tasks.yaml 路径（默认 deploy/tasks.yaml）")
