...
定位（阶段 4，2026-08-14）：本模块 = AgentOS 侧**触发/表达层**（报告写入
沙漏 + observer-alerts，供插件按 topic 注入）。复核的**加工**（不完全线索
再巩固式复核 + 降权/合并结果应用）已下沉进 LMS 做梦引擎（dream_engine.py
_doubt_review 阶段 4 延伸：反教条 top-N 高频复核子过程，DREAM_DOUBT_ENABLED
治理开关默认开）——怀疑是本能，怀疑底座（precision 动态化 + 思考链 + 做梦
复核）持续运作，这里负责把"正在怀疑什么"表达给人看。
