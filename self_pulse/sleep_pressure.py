#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ⚠️ DEPRECATED（2026-08-14 标注，审计 P2-4 遗留副本治理）——————————————
# 本副本为 2026-08-13 旧版（无 refresh 功能），仅存档，勿用、勿改、勿引用！
#
# 生产版：/vol2/1000/AI专用/所有自动化/轻如烟/scripts/sleep_pressure.py
#   （8/14 方案A 起含 refresh()：self_pulse 每 10min 无条件调用，推进状态机+
#    落盘，与唤醒链 check() 解耦——本旧版无此功能，误用会让状态冻结回归
#    8/13 事故形态；且两者同 _STATE_FILE 路径，旧版写入会覆盖 refresh 成果）
#
# 当前零调用者（grep 全仓无引用；cron 指向生产版）。如需使用请 import 生产版。
# ----------------------------------------------------------------------------
"""
...
# 主会话目录（OpenClaw sessions）：机器相关，正式部署经 env.local 的
# RESET_WATCHDOG_SESSIONS_DIR 或 SP_SESSION_DIR 配置；兜底 ~/.openclaw（可移植）。
